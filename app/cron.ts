...
import { deviceInactiveDays } from '@/config'
import { db } from '@/db'
...
async function cleanupInactiveDevices() {
  ...
  for (const { userId, deviceId } of inactive) {
    db.transaction((tx) => {
      tx.run(
        sql`DELETE FROM e2ee_device_keys WHERE user_id = ${userId} AND device_id = ${deviceId}`,
      )
      tx.run(
        sql`DELETE FROM e2ee_one_time_keys WHERE user_id = ${userId} AND device_id = ${deviceId}`,
      )
      tx.run(
        sql`DELETE FROM e2ee_fallback_keys WHERE user_id = ${userId} AND device_id = ${deviceId}`,
      )
      tx.run(
        sql`DELETE FROM e2ee_to_device_messages WHERE user_id = ${userId} AND device_id = ${deviceId}`,
      )
      tx.run(sql`DELETE FROM devices WHERE user_id = ${userId} AND id = ${deviceId}`)
      tx.run(
        sql`INSERT INTO e2ee_device_list_changes (user_id, ulid) VALUES (${userId}, ${generateUlid()})`,
      )
    })
