...
export async function getStateContent(
  ...
  const row = db
    .select({ content: eventsState.content })
    .from(currentRoomState)
    .innerJoin(eventsState, eq(currentRoomState.eventId, eventsState.id))
    .where(
      and(
    ...
    .get()
  const result = (row?.content as Record<string, unknown>) ?? null
...
export function getAllStateEventIds(roomId: string): string[] {
  return db
    .select({ eventId: currentRoomState.eventId })
    .from(currentRoomState)
...
export async function invalidateStateContent(
  roomId: string,
  type?: string,
  stateKey?: string,
): Promise<void> {
