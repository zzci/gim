...
import { oauthAccessTokenCacheMaxTtlSec } from '@/config'
import { db } from '@/db'
...
const ACCESS_TOKEN_CACHE_MAX_TTL_SECONDS = oauthAccessTokenCacheMaxTtlSec
...
async function setAccessTokenHit(row: OAuthAccessTokenRecord, nowMs: number): Promise<void> {
  await cacheSet(accessTokenCacheKey(row.token), toCacheHit(row), {
    ttl: computeAccessTokenTtlSeconds(row, nowMs),
  })
}

async function setAccessTokenMiss(token: string): Promise<void> {
  await cacheSet(accessTokenCacheKey(token), { found: false } satisfies AccessTokenCacheMiss, {
    ttl: ACCESS_TOKEN_MISS_TTL_SECONDS,
  })
}
...
export async function getOAuthAccessToken(token: string): Promise<OAuthAccessTokenRecord | null> {
  ...
  const row = db
    .select()
    .from(oauthTokens)
    .where(and(eq(oauthTokens.id, id), eq(oauthTokens.type, 'AccessToken')))
    .get()
  if (!row) {
...
export async function invalidateOAuthAccessTokensByAccountDevice(
  accountId: string,
  deviceId: string,
): Promise<void> {
  const rows = db
