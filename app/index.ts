...
async function run() {
  ...
  app.get('/metrics', (c) => {
    if (metricsSecret) {
      ...
      if (auth !== `Bearer ${metricsSecret}`) {
        return c.json({ errcode: 'M_UNAUTHORIZED', error: 'Invalid metrics token' }, 401)
      }
    }
  ...
  /* admin */
  app.use('/admin/api/login', adminLoginRateLimit)
  app.route('/admin', adminRoute)
  app.get(
    '/admin/assets/*',
    serveStatic({ root: './admin/dist', rewriteRequestPath: p => p.replace('/admin', '') }),
  )
  ...
  const apis = inspectRoutes(app)
    ...
    .reduce((acc, { method, path }) => {
      ...
      if (existing)
        existing.add(method)
      else acc.set(path, new Set([method]))
      return acc
    }, new Map<string, Set<string>>())

  const apiList = [...apis.entries()].map(([path, methods]) => `${[...methods].join('/')} ${path}`)
  app.get('/', c =>
    c.json({ server: 'gim', version, build: buildInfo, domain: serverName, apis: apiList }))
