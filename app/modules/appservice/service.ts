...
function getRoomAliasesForRooms(roomIds: string[]): Map<string, string[]> {
  ...
  const matchedAliases = db
    .select()
    .from(roomAliases)
    .where(inArray(roomAliases.roomId, roomIds))
    .all()
...
function isAsSenderInRoom(reg: CompiledRegistration, roomId: string): boolean {
  ...
  const membership = db
    .select({ membership: roomMembers.membership })
    .from(roomMembers)
    ...
    try {
      ...
      if (lastPos) {
        stateEvents = db
          .select()
          .from(eventsState)
          .where(gt(eventsState.id, lastPos))
          .orderBy(eventsState.id)
          .limit(BATCH_SIZE)
          .all()
        timelineEvents = db
          .select()
          .from(eventsTimeline)
          .where(gt(eventsTimeline.id, lastPos))
          .orderBy(eventsTimeline.id)
          .limit(BATCH_SIZE)
          .all()
      }
      else {
      ...
      if (matchedEvents.length > 0) {
        ...
        const response = await safeFetch(`${reg.url}/_matrix/app/v1/transactions/${txnId}`, {
          method: 'PUT',
          headers: {
        ...
        if (response.ok) {
          db.update(appservices)
            ...
            .where(eq(appservices.id, reg.id))
            .run()
        }
        else {
      ...
      if (err instanceof SSRFError) {
        logger.warn('appservice_blocked_ssrf', { asId: reg.asId, url: reg.url, error: err.message })
      }
      else {
  ...
  for (const reg of registrations) {
    ...
    try {
      const response = await safeFetch(
        `${reg.url}/_matrix/app/v1/users/${encodeURIComponent(userId)}`,
        {
      ...
      if (err instanceof SSRFError)
        logger.warn('appservice_blocked_ssrf', { asId: reg.asId, error: err.message })
      // AS unreachable or SSRF blocked
    }
  }
...
export async function queryAppServiceRoomAlias(alias: string): Promise<string | null> {
  ...
  for (const reg of registrations) {
    ...
    try {
      const response = await safeFetch(
        `${reg.url}/_matrix/app/v1/rooms/${encodeURIComponent(alias)}`,
        {
          headers: { Authorization: `Bearer ${reg.hsToken}` },
        },
      )
