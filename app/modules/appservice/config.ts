...
function compileNamespaceEntries(
  entries?: { exclusive?: boolean, regex: string }[],
): CompiledNamespaceEntry[] {
  if (!entries)
    return []
...
function importYamlRegistrations(): void {
  ...
  const files = readdirSync(asRegistrationDir).filter(
    f => f.endsWith('.yaml') || f.endsWith('.yml'),
  )

  // Get current max stream position for new registrations
  const stateMax = db
    .select({ id: eventsState.id })
    .from(eventsState)
    .orderBy(sql`id DESC`)
    .limit(1)
    .get()
  const timelineMax = db
    .select({ id: eventsTimeline.id })
    .from(eventsTimeline)
    .orderBy(sql`id DESC`)
    .limit(1)
    .get()
...
function hydrateRegistrationFromDb(
  row: typeof appservices.$inferSelect | undefined,
): CompiledRegistration | undefined {
  if (!row)
    return undefined
  const compiled = compileRow(row)
...
export function getRegistrationByAsToken(token: string): CompiledRegistration | undefined {
  ...
  for (const [asToken, reg] of registrationsByAsToken) {
    const asBuf = Buffer.from(asToken)
    if (tokenBuf.length === asBuf.length && timingSafeEqual(tokenBuf, asBuf))
      return reg
  }
...
export function findInterestedServices(
  event: {
    sender: string
    roomId: string
  ...
  for (const reg of registrationsById.values()) {
    ...
    if (
      event.stateKey
      && event.stateKey.startsWith('@')
      && matchesNamespace(event.stateKey, reg.namespaces.users)
    ) {
      interested.push(reg)
      continue
    }
