...
import { db } from '@/db'
import { accounts, accountTokens, devices, oauthTokens, roomMembers } from '@/db/schema'
...
export function registerAdminUsersRoutes(adminRoute: Hono) {
  ...
  adminRoute.put('/api/users/:userId', async (c) => {
    ...
    // Revoke all tokens when deactivating a user
    if (body.isDeactivated) {
      const localpart = userId.slice(1, userId.lastIndexOf(':'))
      // Invalidate cached OAuth access tokens before deleting from DB
      ...
      // Invalidate any remaining cached tokens
      await cacheDelPrefix('account_token:')
    }
    ...
    if (typeof body.isDeactivated === 'boolean') {
      logAdminAction(
        adminUserId,
        body.isDeactivated ? 'user.deactivate' : 'user.reactivate',
        'user',
        userId,
        updates,
        ip,
    ...
    if (typeof body.admin === 'boolean') {
      logAdminAction(
        adminUserId,
        body.admin ? 'user.grant_admin' : 'user.revoke_admin',
        'user',
        userId,
        updates,
        ip,
