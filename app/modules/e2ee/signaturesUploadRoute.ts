...
signaturesUploadRoute.post('/', async (c) => {
  ...
  for (const [userId, keyMap] of Object.entries(body) as [string, Record<string, any>][]) {
    ...
    if (userId !== auth.userId) {
      ...
      const userSigningKey = db
        .select({ keyData: accountDataCrossSigning.keyData })
      ...
      if (!userSigningKey) {
        for (const keyId of Object.keys(keyMap)) {
          failures[userId][keyId] = {
            errcode: 'M_FORBIDDEN',
            error: 'No user-signing key available',
          }
        }
        continue
      }
      ...
      for (const [keyId, signedObject] of Object.entries(keyMap)) {
        ...
        const matchesTarget = Object.keys(targetKeys).some(
          (k: string) =>
        ...
        // Cryptographically verify the signature using auth user's user-signing key
        const usKeyData = userSigningKey.keyData as any
        const usKeys = (usKeyData.keys as Record<string, string>) || {}
        const usKeyEntry = Object.entries(usKeys).find(([k]) => k.startsWith('ed25519:'))

        if (!usKeyEntry) {
          failures[userId][keyId] = {
            errcode: 'M_FORBIDDEN',
            error: 'User-signing key has no ed25519 key',
          }
          continue
        }

        const [usKeyId, usPublicKey] = usKeyEntry
        const sigResult = verifyEd25519Signature(
          signedObject,
          auth.userId,
          usKeyId,
          usPublicKey as string,
        )
        if (!sigResult.valid) {
          failures[userId][keyId] = {
            errcode: 'M_FORBIDDEN',
        ...
        const newSignatures = (signedObject.signatures || {}) as Record<
          string,
          Record<string, string>
        >
        const existingSigs = ((targetMasterKey.keyData as any).signatures || {}) as Record<
          string,
          Record<string, string>
        >
        ...
        db.update(accountDataCrossSigning)
          ...
          .run()
        logger.debug('signatures_upload_cross_user_master_key', {
      ...
      if (crossUserChanged) {
        db.insert(e2eeDeviceListChanges)
          .values({
            userId,
            ulid: generateUlid(),
          })
    ...
    for (const [keyId, signedObject] of Object.entries(keyMap)) {
      ...
      if (dk) {
        ...
        if (
          !hasNewSignatures(existingSigs, newSignatures as Record<string, Record<string, string>>)
        ) {
          continue
        }

        const merged = { ...existingSigs }
        for (const [sigUserId, sigs] of Object.entries(newSignatures) as [
          string,
          Record<string, string>,
        ][]) {
        ...
        db.update(e2eeDeviceKeys)
          ...
          .run()
        logger.debug('signatures_upload_applied_device_key', {
          userId,
          keyId,
          matchedDeviceId: dk.deviceId,
        })
        ...
        if (userId === auth.userId) {
          ...
          if (selfSigningKey) {
            const ssKeys = (selfSigningKey.keyData as any).keys as
              | Record<string, string>
              | undefined
            if (ssKeys) {
              ...
              if (ssKeyEntry) {
                ...
                if (sigResult.valid) {
                  ...
                  logger.info('device_trust_promoted_by_self_signing', {
                    userId,
      ...
      const csKeys = db
        ...
        .from(accountDataCrossSigning)
        .where(eq(accountDataCrossSigning.userId, userId))
        .all()
      ...
      for (const csk of csKeys) {
        ...
        if (
          ...
          const existingCsSigs = (keyData.signatures || {}) as Record<
            string,
            Record<string, string>
          >
          if (
            !hasNewSignatures(
              existingCsSigs,
              newSignatures as Record<string, Record<string, string>>,
            )
          ) {
            matched = true
            break
          ...
          db.update(accountDataCrossSigning)
            ...
            .run()
          logger.debug('signatures_upload_applied_cross_signing_key', {
            userId,
            keyId,
            matchedKeyType: csk.keyType,
          })
          matched = true
          anyChanged = true
          break
