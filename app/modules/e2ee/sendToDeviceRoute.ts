...
} from '@/modules/e2ee/verificationSessions'
...
sendToDeviceRoute.put('/:eventType/:txnId', async (c) => {
  ...
  if (auth.trustState !== 'trusted' && !verificationEvent) {
    return matrixError(c, 'M_FORBIDDEN', 'Device is not verified', {
      errcode_detail: 'M_DEVICE_UNVERIFIED',
    })
  }
    ...
    for (const [deviceId, content] of Object.entries(deviceMap)) {
      if (deviceId === '*') {
        ...
        for (const d of userDevices) {
          const target = db
            .select({ trustState: devices.trustState })
            .from(devices)
          ...
          db.insert(e2eeToDeviceMessages)
            .values({
              userId,
              deviceId: d.id,
              type: eventType,
  ...
  // Trust promotion is handled exclusively via cross-signing self_signing_key
  // signatures in signaturesUploadRoute.ts (not via to-device event types)
