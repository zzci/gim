...
import {
  createCrossSigningChallenge,
  CROSS_SIGNING_KEY_TYPES,
  isCrossSigningResetVerified,
  stableJson,
} from './crossSigningHelpers'
...
crossSigningRoute.post('/', async (c) => {
  ...
  for (const { field, usage, dbType } of keyTypes) {
    ...
    if (
      !keyData.signatures
      || typeof keyData.signatures !== 'object'
      || Object.keys(keyData.signatures).length === 0
    ) {
    ...
    if (ed25519Keys.length !== 1) {
      return matrixError(
        c,
        'M_INVALID_PARAM',
        `Expected exactly one ed25519 key in ${field}, found ${ed25519Keys.length}`,
      )
    }
    ...
    if (keyData.user_id && keyData.user_id !== auth.userId) {
      return matrixError(
        c,
        'M_INVALID_PARAM',
        `user_id in ${field} does not match authenticated user`,
      )
    }
  ...
  const existingByType = new Map(
    existingRows.map(r => [r.keyType as CrossSigningDbType, r.keyData]),
  )
  ...
  if (existingRows.length > 0) {
    ...
    if (body.reset !== true) {
      return matrixError(
        c,
        'M_FORBIDDEN',
        'Cross-signing metadata already exists; reset=true is required',
      )
    }

    if (!isCrossSigningResetVerified(c.req.header('Authorization'), auth.userId, body.auth)) {
      const session = createCrossSigningChallenge(auth.userId)
      return c.json(
        {
          flows: [{ stages: ['m.login.reauth'] }],
          session,
        },
        401,
      )
    ...
    if (missingTypes.length > 0) {
      return matrixError(
        c,
        'M_INVALID_PARAM',
        `Cross-signing reset must provide full metadata, missing: ${missingTypes.join(', ')}`,
      )
    }
    ...
    db.transaction((tx) => {
      ...
      for (const { dbType, keyData } of incoming) {
        tx.insert(accountDataCrossSigning)
          .values({
            userId: auth.userId,
            keyType: dbType,
            keyData,
    ...
    for (const { dbType, keyData } of incoming) {
      await db
        .insert(accountDataCrossSigning)
        .values({
          userId: auth.userId,
          keyType: dbType,
          keyData,
