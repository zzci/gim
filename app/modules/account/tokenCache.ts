...
import { accountTokenCacheMaxTtlSec, accountTokenValiditySec } from '@/config'
import { db } from '@/db'
...
const TOKEN_CACHE_MAX_TTL_SECONDS = accountTokenCacheMaxTtlSec
const TOKEN_VALIDITY_SECONDS = accountTokenValiditySec
...
async function setTokenCache(row: AccountTokenRecord, nowMs: number): Promise<void> {
  await cacheSet(tokenCacheKey(row.token), toCacheEntry(row), {
    ttl: computeCacheTtlSeconds(row, nowMs),
  })
}

async function setTokenMissCache(token: string): Promise<void> {
  await cacheSet(tokenCacheKey(token), { found: false } satisfies AccountTokenCacheMiss, {
    ttl: TOKEN_MISS_TTL_SECONDS,
  })
}
...
export async function markAccountTokenUsed(token: string, usedAtMs = Date.now()): Promise<void> {
  ...
  await cacheSet(tokenCacheKey(token), cached, {
    ttl: computeCacheTtlSeconds(fromCacheEntry(cached), usedAtMs),
  })
}
