...
export function prefetchBatchSyncData(
  roomIds: string[],
  userId: string,
  sinceId: string | null,
  sinceStreamId: string | null = null,
): BatchSyncData {
  const memberCounts = new Map<string, { joined: number, invited: number }>()
  const heroes = new Map<string, string[]>()
  const receipts = new Map<
    string,
    Array<{ userId: string, eventId: string, receiptType: string, ts: number }>
  >()
  ...
  for (const chunk of chunkArray(roomIds, SQLITE_MAX_PARAMS)) {
    const countRows = sqlite
      ...
      FROM room_members
      WHERE room_id IN (${chunk.map(() => '?').join(',')})
      GROUP BY room_id
    `,
      )
      .all(...chunk) as Array<{ room_id: string, joined: number, invited: number }>
  ...
  for (const chunk of chunkArray(roomIds, SQLITE_MAX_PARAMS - 1)) {
    const heroRows = sqlite
      .prepare(
        `
    ...
    `,
      ...
      .all(...chunk, userId) as Array<{ room_id: string, user_id: string }>
  ...
  for (const chunk of chunkArray(roomIds, SQLITE_MAX_PARAMS - 1)) {
    const unreadRows = sqlite
      ...
      WHERE et.room_id IN (${chunk.map(() => '?').join(',')})
        AND (rr.event_id IS NULL OR et.id > rr.event_id)
      GROUP BY et.room_id
    `,
      )
      .all(userId, ...chunk) as Array<{ room_id: string, cnt: number }>
  ...
  for (const chunk of chunkArray(roomIds, SQLITE_MAX_PARAMS)) {
    ...
    `,
      ...
      .all(...chunk) as Array<{ room_id: string, user_id: string }>

    for (const row of typingRows) {
      const existing = typing.get(row.room_id)
      if (existing)
        existing.push(row.user_id)
      else typing.set(row.room_id, [row.user_id])
    }
  }

  // Batch room account data (filter by stream_id for incremental sync)
  const accountDataChunkSize = sinceStreamId ? SQLITE_MAX_PARAMS - 2 : SQLITE_MAX_PARAMS - 1
  for (const chunk of chunkArray(roomIds, accountDataChunkSize)) {
    const accountDataQuery = sinceStreamId
      ? sqlite.prepare(`
          SELECT user_id, type, room_id, content
          FROM account_data
          WHERE user_id = ? AND room_id IN (${chunk.map(() => '?').join(',')}) AND stream_id > ?
        `)
      : sqlite.prepare(`
          SELECT user_id, type, room_id, content
          FROM account_data
          WHERE user_id = ? AND room_id IN (${chunk.map(() => '?').join(',')})
        `)
    const accountDataRows = (
      sinceStreamId
        ? accountDataQuery.all(userId, ...chunk, sinceStreamId)
        : accountDataQuery.all(userId, ...chunk)
    ...
    for (const row of accountDataRows) {
      ...
      if (existing)
        existing.push(entry)
      else roomAccountData.set(row.room_id, [entry])
    }
  }
...
export function buildJoinedRoomData(
  ...
  if (sinceId === null) {
    ...
    roomEvents = queryRoomEvents(roomId, {
      after: sinceId,
      order: 'asc',
    ...
    if (roomEvents.length > DEFAULT_TIMELINE_LIMIT) {
      limited = true
      roomEvents = roomEvents.slice(0, DEFAULT_TIMELINE_LIMIT)
    }
...
export function buildInviteRoomData(
  ...
  const inviteEvents
    = inviteStateIds.length > 0
      ? getStateEventsByIds(inviteStateIds)
          .filter(event => INVITE_STATE_TYPES.includes(event.type))
          .map(formatEvent)
      : []
