...
export function buildSyncResponse(opts: SyncOptions) {
  ...
  // Clean expired typing notifications (throttled to once every 5s)
  if (Date.now() - lastTypingCleanup > 5000) {
    lastTypingCleanup = Date.now()
  ...
  const batchData = prefetchBatchSyncData(
    joinedRoomIds,
    opts.userId,
    trustedSinceId,
    trustedSinceId,
  ...
  const toDevice = collectToDeviceMessages(
    opts.userId,
    opts.deviceId,
    trust.isTrusted,
    sinceId !== null,
