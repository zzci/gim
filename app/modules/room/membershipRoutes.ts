...
roomMembershipRouter.post('/:roomId/join', async (c) => {
  ...
  if (!(await checkRoomMemberLimit(roomId))) {
    return matrixError(
      c,
      'M_RESOURCE_LIMIT_EXCEEDED',
      `This room has reached the maximum number of members (${maxRoomMembers})`,
    )
  }
...
roomMembershipRouter.get('/:roomId/members', async (c) => {
  ...
  const eventIds = filtered.map(r => r.eventId).filter(Boolean)
  const events
    = eventIds.length > 0
      ? chunkArray(eventIds, 999).flatMap(chunk =>
          db.select().from(eventsState).where(inArray(eventsState.id, chunk)).all(),
        )
      : []

  const eventsMap = new Map(events.map(e => [e.id, e]))
  const memberEvents = filtered
    .map((row) => {
...
roomMembershipRouter.get('/:roomId/aliases', async (c) => {
  ...
  const aliases = db
    .select({ alias: roomAliases.alias })
    .from(roomAliases)
