...
const VALID_SCHEME = /^[a-z][a-z0-9+\-.]*$/i
...
function isValidRedirectUrl(url: string): boolean {
  try {
    ...
    if (scheme === 'http:' && (parsed.hostname === 'localhost' || parsed.hostname === '127.0.0.1'))
      return true
    // Allow custom URI schemes for mobile apps (e.g. element://, io.element.app://)
    const schemeName = scheme.slice(0, -1) // strip trailing ':'
    if (schemeName && VALID_SCHEME.test(schemeName))
      return true
    return false
  }
  catch {
