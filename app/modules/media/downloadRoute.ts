...
async function handleDownload(c: any) {
  ...
  const isSafeMime
    = /^(?:image|video|audio)\//.test(record.contentType) || record.contentType === 'application/pdf'
  ...
  const disposition = isSafeMime ? 'inline' : 'attachment'
  if (record.fileName) {
    const safe = sanitizeFileName(record.fileName)
    headers['Content-Disposition'] = `${disposition}; filename*=UTF-8''${encodeURIComponent(safe)}`
  }
  else if (!isSafeMime) {
