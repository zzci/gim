...
mediaUploadRoute.post('/', async (c) => {
  ...
  if (!checkUploadRateLimit(auth.userId)) {
    return matrixError(
      c,
      'M_LIMIT_EXCEEDED',
      `Upload rate limit exceeded (${mediaUploadsPerHour}/hour)`,
...
mediaUploadRoute.put('/:server/:mediaId', async (c) => {
  ...
  if (!/^[\w-]{8,64}$/.test(mediaId))
    return matrixError(c, 'M_INVALID_PARAM', 'Invalid media ID format')
  ...
  const existingMedia = db
    ...
    .get()
  if (existingMedia && existingMedia.userId !== auth.userId)
    return matrixError(c, 'M_FORBIDDEN', 'Media ID belongs to another user')
