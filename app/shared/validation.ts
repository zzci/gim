...
export const matrixUserId = z
  .string()
  .regex(/^@[a-z0-9._=/+-]+:[a-zA-Z0-9.-]+$/, 'Invalid Matrix user ID format')

export const eventContent = z
  .record(z.string(), z.unknown())
  .refine(v => JSON.stringify(v).length <= 65535, 'Event content exceeds 64KB limit')

export const displayName = z.string().max(256, 'Display name must be 256 characters or fewer')

export const avatarUrl = z
  .string()
  .max(1024, 'Avatar URL must be 1024 characters or fewer')
  .startsWith('mxc://', 'Avatar URL must start with mxc://')
...
export const createRoomBody = z.object({
  ...
  initial_state: z
    .array(
      z.object({
        type: z.string(),
        state_key: z.string().optional().default(''),
...
export function validate<T>(
  ...
  if (!result.success) {
    ...
    return {
      success: false,
      response: matrixError(c, 'M_BAD_JSON', message) as unknown as Response,
    }
  }
