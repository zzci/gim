...
export const accountTokens = sqliteTable(
  ...
  table => [index('account_tokens_user_id_idx').on(table.userId)],
)

export const accountData = sqliteTable(
  'account_data',
  {
    userId: text('user_id')
      .notNull()
  ...
  table => [
    ...
    index('account_data_user_room_stream_idx').on(table.userId, table.roomId, table.streamId),
  ],
)

export const accountDataCrossSigning = sqliteTable(
  'account_data_cross_signing',
  {
    userId: text('user_id')
      .notNull()
      .references((): AnySQLiteColumn => accounts.id),
  ...
  table => [primaryKey({ columns: [table.userId, table.keyType] })],
)

export const accountFilters = sqliteTable(
  'account_filters',
  {
  ...
  table => [index('account_filters_user_id_idx').on(table.userId)],
)
...
export const roomMembers = sqliteTable(
  'room_members',
  {
    roomId: text('room_id')
      .notNull()
      .references((): AnySQLiteColumn => rooms.id),
    userId: text('user_id')
      .notNull()
      .references((): AnySQLiteColumn => accounts.id),
...
export const eventsState = sqliteTable(
  'events_state',
  {
  ...
  table => [index('events_state_room_id_idx').on(table.roomId, table.id)],
)

export const eventsTimeline = sqliteTable(
  'events_timeline',
  {
    id: text('id').primaryKey(), // ULID (= ordering key)
    roomId: text('room_id').notNull(),
  ...
  table => [index('events_timeline_room_id_idx').on(table.roomId, table.id)],
)

export const eventsAttachments = sqliteTable(
  'events_attachments',
  {
    id: text('id').primaryKey().$defaultFn(generateUlid),
    eventId: text('event_id').notNull(),
...
export const currentRoomState = sqliteTable(
  'current_room_state',
  {
    roomId: text('room_id')
      .notNull()
    ...
    eventId: text('event_id')
      .notNull()
      .references((): AnySQLiteColumn => eventsState.id),
  },
  table => [
...
export const e2eeDeviceKeys = sqliteTable(
  'e2ee_device_keys',
  {
    ...
    signatures: text('signatures', { mode: 'json' })
      .notNull()
...
export const e2eeOneTimeKeys = sqliteTable(
  'e2ee_one_time_keys',
  {
    id: text('id').primaryKey().$defaultFn(generateUlid),
    userId: text('user_id').notNull(),
...
export const e2eeFallbackKeys = sqliteTable(
  'e2ee_fallback_keys',
  {
    userId: text('user_id').notNull(),
    deviceId: text('device_id').notNull(),
  ...
  table => [primaryKey({ columns: [table.userId, table.deviceId, table.algorithm] })],
)

export const e2eeToDeviceMessages = sqliteTable(
  'e2ee_to_device_messages',
  {
  ...
  table => [index('e2ee_to_device_user_device_idx').on(table.userId, table.deviceId)],
)

export const e2eeDeviceListChanges = sqliteTable(
  'e2ee_device_list_changes',
  {
    id: integer('id').primaryKey({ autoIncrement: true }),
...
export const e2eeRoomKeyBackups = sqliteTable(
  'e2ee_room_key_backups',
  {
    userId: text('user_id')
      .notNull()
      .references((): AnySQLiteColumn => accounts.id),
...
export const e2eeRoomKeyBackupKeys = sqliteTable(
  'e2ee_room_key_backup_keys',
  {
    userId: text('user_id')
      .notNull()
      .references((): AnySQLiteColumn => accounts.id),
...
export const mediaDeletions = sqliteTable(
  'media_deletions',
  {
    id: text('id').primaryKey().$defaultFn(generateUlid),
    mediaId: text('media_id').notNull(),
  ...
  table => [index('media_deletions_completed_at_idx').on(table.completedAt)],
)
...
export const pushNotifications = sqliteTable(
  ...
  table => [index('push_notifications_user_id_idx').on(table.userId)],
)

export const pushers = sqliteTable(
  'pushers',
  {
    id: text('id').primaryKey().$defaultFn(generateUlid),
    userId: text('user_id')
  ...
  table => [index('pushers_user_id_idx').on(table.userId)],
)

export const pushRules = sqliteTable(
  'push_rules',
  {
    id: text('id').primaryKey().$defaultFn(generateUlid),
    userId: text('user_id')
  ...
  table => [index('push_rules_user_id_idx').on(table.userId)],
)
...
export const readReceipts = sqliteTable(
  ...
  table => [primaryKey({ columns: [table.roomId, table.userId, table.receiptType] })],
)

export const typingNotifications = sqliteTable(
  ...
  table => [
    primaryKey({ columns: [table.roomId, table.userId] }),
    index('typing_notifications_expires_at_idx').on(table.expiresAt),
  ],
)
...
export const presence = sqliteTable(
  ...
  table => [index('presence_state_last_active_idx').on(table.state, table.lastActiveAt)],
)
